...
import { cascadiaText, type CascadiaTranslationKey } from '../i18n';
import {
  computeHabitatMajorityTotals,
...
const SCORE_SECTIONS: Array<{
  titleKey: CascadiaTranslationKey;
  categories: Array<{
    key: string;
    translationKey: CascadiaTranslationKey;
  }>;
}> = [
  {
    titleKey: 'sectionWildlife',
    categories: [
      { key: 'bears', translationKey: 'bears' },
      { key: 'elk', translationKey: 'elk' },
      { key: 'salmon', translationKey: 'salmon' },
      { key: 'hawks', translationKey: 'hawks' },
      { key: 'foxes', translationKey: 'foxes' },
    ],
  },
  {
    titleKey: 'sectionHabitats',
    categories: [
      { key: 'mountains', translationKey: 'mountains' },
      { key: 'forests', translationKey: 'forests' },
      { key: 'prairies', translationKey: 'prairies' },
      { key: 'wetlands', translationKey: 'wetlands' },
      { key: 'rivers', translationKey: 'rivers' },
    ],
  },
  {
    titleKey: 'sectionOther',
    categories: [{ key: 'natureTokens', translationKey: 'natureTokens' }],
  },
];

const INPUT_SCORE_KEYS = [
  ...WILDLIFE_KEYS,
  ...HABITAT_KEYS,
  'natureTokens',
] as const;

type ScoreCategoryKey = (typeof INPUT_SCORE_KEYS)[number];
type ScoreInputs = Record<ScoreCategoryKey, string>;

interface CascadiaPlayerInput {
  name: string;
  email: string | null;
  scores: ScoreInputs;
}

interface CascadiaRecordFormProps {
  game: GameConfig;
  mode: 'create' | 'edit';
  recordId?: string;
  userEmail: string;
...
function createEmptyScores(): ScoreInputs {
  return Object.fromEntries(
    INPUT_SCORE_KEYS.map((key) => [key, '']),
  ) as ScoreInputs;
}

function createEmptyPlayer(): CascadiaPlayerInput {
  return {
    name: '',
    email: null,
...
function toPlayerInput(player: PlayerScore): CascadiaPlayerInput {
  const scores = createEmptyScores();
  for (const key of INPUT_SCORE_KEYS) {
...
export function CascadiaRecordForm({
  game,
  mode,
  recordId,
  userEmail,
}: CascadiaRecordFormProps) {
  ...
  const [players, setPlayers] = useState<CascadiaPlayerInput[]>([
    createEmptyPlayer(),
  ]);
  ...
  const majorityTotals = computeHabitatMajorityTotals(
    players.map((player) => player.scores),
  );
  const playerTotals = players.map((player, index) =>
    computePlayerScoreTotals(player.scores, majorityTotals[index] ?? 0),
  );
  ...
  function updatePlayer(
    index: number,
    update: (player: CascadiaPlayerInput) => CascadiaPlayerInput,
  ) {
    setPlayers((current) =>
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (players.some((player) => player.name.trim().length === 0)) {
      setError(cascadiaText(locale, 'missingPlayerName'));
      return;
    }

    const hasInvalidScore = players.some((player) =>
      INPUT_SCORE_KEYS.some((key) => {
    ...
    if (hasInvalidScore) {
      setError(cascadiaText(locale, 'invalidScores'));
      return;
    }

    const majorityOnSave = computeHabitatMajorityTotals(
      players.map((player) => player.scores),
    );

    const normalizedPlayers: PlayerScore[] = players.map((player, index) => {
      const majority = majorityOnSave[index] ?? 0;
      const totals = computePlayerScoreTotals(player.scores, majority);
      const scoreBreakdown = {
        ...Object.fromEntries(
          INPUT_SCORE_KEYS.map((key) => [key, Number(player.scores[key]) || 0]),
        ),
        habitatMajority: majority,
      };
      return {
    ...
    <section className="max-w-2xl">
      <div className="mb-6 flex items-center justify-between gap-3">
        <h2 className="text-lg font-semibold">
          {mode === 'edit' ? t('records.editTitle') : t('records.addTitle')}
        </h2>
        <Link
          to={`/game/${game.id}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          {players.map((player, playerIndex) => {
            const totals = playerTotals[playerIndex];
            return (
              <fieldset
                key={playerIndex}
              ...
              >
                ...
                <div className="mb-4 space-y-1.5">
                  <span className="text-xs text-neutral-500 dark:text-neutral-400">
                    {cascadiaText(locale, 'playerName')}
                  </span>
                  <PlayerNameField
                    id={`cascadia-player-${playerIndex}`}
                    value={player.name}
                  ...
                  {SCORE_SECTIONS.map((section) => (
                    <div key={section.titleKey} className="space-y-2">
                      <h4 className="text-xs font-medium tracking-wide text-neutral-500 uppercase dark:text-neutral-400">
                        {cascadiaText(locale, section.titleKey)}
                      </h4>
                      <div className="grid gap-3 sm:grid-cols-2">
                        {section.categories.map(({ key, translationKey }) => (
                          <label key={key} className="space-y-1.5">
                            <span className="inline-flex items-center gap-1.5 text-xs text-neutral-500 dark:text-neutral-400">
                              ...
                              {cascadiaText(locale, translationKey)}
                            </span>
                            <input
                              type="number"
                              min={0}
                              value={player.scores[key as ScoreCategoryKey]}
                              onChange={(event) => {
                      ...
                      {section.titleKey === 'sectionWildlife' ? (
                        <p className="text-right text-xs font-medium text-neutral-600 dark:text-neutral-300">
                          {cascadiaText(locale, 'subtotalWildlife', {
                            points: totals.wildlife,
                          })}
                        </p>
                      ) : null}
                      {section.titleKey === 'sectionHabitats' ? (
                        <div className="space-y-1 text-right text-xs font-medium text-neutral-600 dark:text-neutral-300">
                          <p>
                            {cascadiaText(locale, 'subtotalHabitats', {
                              points: totals.habitats,
                            })}
                          </p>
                          <p>
                            {cascadiaText(locale, 'subtotalMajority', {
                              points: totals.majority,
                            })}
                          </p>
                          <p className="font-normal text-neutral-400 dark:text-neutral-500">
                            {cascadiaText(locale, 'subtotalMajorityHint')}
                          </p>
                        </div>
                      ) : null}
                      {section.titleKey === 'sectionOther' ? (
                        <p className="text-right text-xs font-medium text-neutral-600 dark:text-neutral-300">
                          {cascadiaText(locale, 'subtotalNature', {
                            points: totals.natureTokens,
                          })}
                        </p>
                      ) : null}
                    </div>
